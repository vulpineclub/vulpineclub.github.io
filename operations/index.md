...
- Using S3 server-side encryption (.env.prooduction, config/initializers/paperclip.rb)
- Not using the stack-fix.c workaround (Dockerfile)
- Using CloudFlare as CDN_HOST (.env.production, Dockerfile)
- HTTP request byte limit is 2 MB instead of 1 MB (app/lib/request.rb, spec/lib/request_spec.rb)
- Image attachment limit is 12 MB instead of 8 MB (app/models/media_attachment.rb)
- Audio attachments are limited as videos (40 MB) instead of as images (app/models/media_attachment.rb)
- Audio attachments have no time limit, instead of the 60 second default (lib/paperclip/audio_transcoder.rb)
- The #timelinemute hashtag prevents a post from appearing in the public timelines locally, while still federating with public visibility (app/models/status.rb, app/services/fan_out_on_writ4e_service.rb, spec/models/status_spec.rb)
- X-Clacks-Overhead header is generated by nginx, not Mastodon (config/environments/production.rb)
